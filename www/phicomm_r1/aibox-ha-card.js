class AiBoxHaCard extends HTMLElement {
  constructor() {
    super();
  ...
  setConfig(config) {
    if (!config || (!config.entity && !Array.isArray(config.rooms))) {
      throw new Error("AI BOX HA Card: cần 'entity' hoặc 'rooms'");
    }
    const rooms = Array.isArray(config.rooms) && config.rooms.length
      ? config.rooms
      : [{ entity: config.entity, name: config.title || "AI BOX" }];
    this._config = {
      title: "AI BOX",
...
if (!window.customCards.find((card) => card.type === "aibox-ha-card")) {
  window.customCards.push({
    type: "aibox-ha-card",
    name: "AI BOX HA Card",
    description: "Standalone AI BOX card (không phụ thuộc card khác), hỗ trợ multi-room cơ bản.",
    preview: false,
  });
}
