; (function () { // IIFE: scope all constants to avoid conflicts with other cards
  // ═══════════════════════════════════════════════════════════════════
  // AI BOX HA LITE Card — Media-only — Cache-busting: force HA to serve latest version
  ...
  class AiBoxLiteCard extends HTMLElement {
    ...
    setConfig(config) {
      ...
      this._state = {
        ...
        media: {
          source: null, isPlaying: false, title: "Không có nhạc", artist: "---",
...
.mc-icon-btn{width:28px;height:28px;border-radius:6px;border:none;background:transparent;color:rgba(225,232,240,.6);cursor:pointer;font-size:14px;display:grid;place-items:center;transition:all .15s}
.mc-icon-btn:hover{background:rgba(255,255,255,.1);color:#fff}
.mc-icon-btn.active-btn{color:#86efac}
.mc-vis{position:relative;border-radius:18px;overflow:hidden;margin-bottom:12px;border:1px solid rgba(255,255,255,0.08);background:#050712;display:flex;flex-direction:column;box-shadow:0 12px 40px rgba(0,0,0,0.6);aspect-ratio:16/9;}
...
.mc-vis-content{position:relative;z-index:2;display:flex;flex-direction:column;height:100%;padding:4px 12px 2px 12px;flex:1;}
.mc-header-overlay{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:0;}
.mc-info-ov{flex:1;min-width:0;margin-right:12px;}
.mc-title-ov{font-size:14px;font-weight:900;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-shadow:0 2px 8px rgba(0,0,0,0.6);margin-bottom:2px;}
.mc-artist-ov{font-size:10px;color:rgba(255,255,255,0.6);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-weight:600;}
.mc-badges-ov{display:flex;flex-direction:column;align-items:flex-end;gap:8px;}
.mc-source-ov{font-size:8px;padding:3px 8px;border-radius:5px;background:rgba(0,0,0,0.5);color:#fff;font-weight:800;letter-spacing:1.2px;text-transform:uppercase;border:1px solid rgba(255,255,255,0.1);}
.mc-opt-btns{display:flex;gap:10px;}
.mc-top-ov{display:flex;align-items:center;gap:24px;margin-bottom:0;margin-top:auto;}
...
.media-controls{display:flex;align-items:center;justify-content:center;gap:20px;margin:0}
.ctrl-btn{width:38px;height:38px;border:none;background:transparent;color:rgba(255,255,255,.7);cursor:pointer;font-size:20px;display:grid;place-items:center;transition:all .15s;border-radius:8px}
.ctrl-btn:hover{background:rgba(255,255,255,.1);color:#fff}
.ctrl-btn.play{width:50px;height:50px;font-size:26px;color:#fff;background:transparent;border:none;box-shadow:none}
.ctrl-btn.stop{color:rgba(239,68,68,.7)}
.ctrl-btn.active-btn{color:#86efac}
...
`;
    ...
    _panelMedia(tab) {
      ...
      return `
<div class="panel active" id="p-media">
  ...
  <div id="searchResults" class="search-results"></div>
    ...
    _bind() {
      ...
      this._buildEqBands();
      this.querySelectorAll("[data-pr]").forEach(b => {
        b.onclick = () => {
      ...
      this.querySelectorAll("[data-sur]").forEach(b => {
        b.onclick = () => {
...
<div class="fx g4 mt8"><button class="form-btn" id="alCancel" style="flex:1">Hủy</button><button class="form-btn green" id="alSubmit" style="flex:1">${isEdit ? "Lưu" : "Thêm"}</button></div>
  ...
  if (typeof PhicommBaseCard !== 'undefined') {
    ...
    if (!customElements.get("aibox-ha-card")) {
      customElements.define("aibox-ha-card-lite", AiBoxHaCard);
    }
  } else if (!customElements.get("aibox-ha-card")) {
    customElements.define("aibox-ha-card-lite", AiBoxLiteCard);
  }
  // Backwards compatibility alias
  if (!customElements.get("aibox-webui-card")) {
    customElements.define("aibox-webui-card-lite", AiBoxLiteCard);
  }
